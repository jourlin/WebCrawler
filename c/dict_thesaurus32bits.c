...
typedef struct LexemeInfo
{
	...
	uint32		posinsubst;		/* pos info in entry */
	uint32		tnvariant;		/* total num lexemes in one variant */
...
typedef struct
{
	uint32		lastlexeme;		/* number lexemes to substitute */
	uint32		reslen;
...
PG_FUNCTION_INFO_V1(thesaurus_init32);
Datum		thesaurus_init32(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(thesaurus_lexize32);
Datum		thesaurus_lexize32(PG_FUNCTION_ARGS);

static void
newLexeme(DictThesaurus *d, char *b, char *e, uint32 idsubst, uint32 posinsubst)
{
	TheLexeme  *ptr;
...
static void
addWrd(DictThesaurus *d, char *b, char *e, uint32 idsubst, uint32 nwrd, uint32 posinsubst, bool useasis)
{
	static int	nres = 0;
	static int	ntres = 0;
	TheSubstitute *ptr;
...
static TheLexeme *
addCompiledLexeme(TheLexeme *newwrds, int *nnw, int *tnm, TSLexeme *lexeme, LexemeInfo *src, uint32 tnvariant)
{
	if (*nnw >= *tnm)
	{
		*tnm *= 2;
...
Datum
thesaurus_init32(PG_FUNCTION_ARGS)
{
	...
	/* Not to be checked when used as an extension */
	/*
	if (!fileloaded)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
static LexemeInfo *
findVariant(LexemeInfo *in, LexemeInfo *stored, uint32 curpos, LexemeInfo **newin, int newn)
{
	for (;;)
	{
		int			i;
...
static TSLexeme *
checkMatch(DictThesaurus *d, LexemeInfo *info, uint32 curpos, bool *moreres)
{
	*moreres = false;
	while (info)
	{
...
Datum
thesaurus_lexize32(PG_FUNCTION_ARGS)
{
